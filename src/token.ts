...
exports.create = fig => validate({
  ...
  content: ['any']
}, fig).then(() => jwt.sign(fig.content, fig.password, _.extend(_.omit(fig, 'content', 'password', 'expiresInSeconds'), {
  algorithm: 'HS256',
...
}, fig).then(() => Q.Promise((resolve, reject) => jwt.verify(fig.token, fig.password, {
  algorithms: ['HS256']
}, (err, decoded) => {
  if (err) {
    reject(err);
