...
module.exports = fig => {
  ...
  self.register = fig => validate(schema(), fig).then(() => dataModel.findByField(usernameField, fig.username)).then(userData => userData && Q.reject(new ValidationError(400, {
    message: `The supplied ${usernameField} has already been taken`
  }))).then(() => emailField && emailField !== usernameField ? dataModel.findByField(emailField, fig[emailField]).then(userData => userData && Q.reject(new ValidationError(400, {
    message: 'The supplied ' + emailField + ' has already been taken'
  }))) : Q()).then(() => password.hash(fig.password)).then(hashedPassword => dataModel.insert(_.extend(_.clone(fig), {
  ...
  self.login = fig => {
    ...
    return validate(validateFig, fig).then(() => dataModel.findByField(usernameField, fig[usernameOrEmailField])).then(userData => !userData && emailField && emailField !== usernameField ? dataModel.findByField(emailField, fig[usernameOrEmailField]) : userData).then(userData => userData || Q.reject(loginError)).then(userData => {
      return password.compare(fig.password, userData.password).then(isMatch => isMatch || Q.reject(loginError)).then(() => {
        const content = {
          type: 'login'
        };
  ...
  self.validateLoginToken = tokenString => token.decode({
    password: tokenSecret,
    token: tokenString
  }).then(decryptedToken => decryptedToken.type === 'login' ? decryptedToken : Q.reject(new ValidationError(400, {
  ...
  if (passwordResetModel) {
    self.sendPasswordReset = username => dataModel.findByField(usernameField, username).then(userData => userData || Q.reject(new ValidationError(400, {
  ...
  }, fig).then(() => token.decode({
    password: tokenSecret,
    token: fig.token
  })).then(tokenData => tokenData.type === 'password-reset' ? tokenData[usernameField] : Q.reject(new ValidationError(400, {
    message: 'Invalid type'
  }))).then(username => {
  ...
  if (confirmationModel) {
    self.sendConfirmation = username => dataModel.findByField(usernameField, username).then(userData => userData || Q.reject(new ValidationError(400, {
      message: 'User not found'
    }))).then(userData => userData.isConfirmed ? Q.reject(new ValidationError(400, {
  ...
  }).then(tokenData => tokenData.type === 'confirmation' ? tokenData[usernameField] : Q.reject(new ValidationError(400, {
    message: 'Invalid type'
  }))).then(username => {
    const setFig = {
      isConfirmed: true
